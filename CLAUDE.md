...
TIL 파일은 `TIL/YYYY/MM/YYYY-MM-DD.md`에 저장 (연/월 폴더로 관리).
하루 기록은 **"오늘 한 일" 요약 테이블 하나**로 관리한다. 두괄식 산문 설명은 쓰지 않는다 — "딱 뭐 했는지"만 남긴다.

파일 형식 (4칸 들여쓴 부분이 실제 파일 내용):

    # TIL - YYYY-MM-DD (요일)

    | # | 한 일 | 참고 |
    |---|-------|------|
    | 1 | <한 일 한 줄 요약> | <커밋 해시 / 문서 경로 / PR 등, 없으면 -> |
    | 2 | ... | ... |

**규칙:**
- "한 일"은 **한 줄**로 무엇을 했는지만 (배경·설명·코드 예시 금지).
- "참고"는 커밋 해시·문서 경로 등 추적 단서. 없으면 `-`.
- 하루 여러 건이면 **같은 테이블에 행만 추가**한다 (헤더·기존 행 보존, 재작성 금지).
- 깊은 학습/설명이 필요하면 TIL이 아니라 `content/{domain}/` wiki 페이지로 작성한다.
